...
#    NOTE: relative to this file (cs229_gnn/4_layer/)
# -------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, "..", "Dataset")

TRAIN_JSONL = os.path.join(DATA_DIR, "all_features_with_mac_part1_80.jsonl")
TEST_JSONL  = os.path.join(DATA_DIR, "all_features_with_mac_part2_20.jsonl")
DEVICES_TXT = os.path.join(DATA_DIR, "List_Of_Devices.txt")
...
# -------------------------
# 6. Build edges (k random neighbors per device label, undirected)
# -------------------------
...
# -------------------------
# 9. 4-layer Deep GraphSAGE model (PyG)
# -------------------------
from torch_geometric.nn import SAGEConv
from torch_geometric.nn.norm import BatchNorm

class SAGEModelDeep4Layer(nn.Module):
...
if __name__ == "__main__":
    ...
    # Save metrics to CSV (4-layer specific name)
    metrics_df = pd.DataFrame(history)
    metrics_path = os.path.join(BASE_DIR, "metrics_4layer.csv")
    ...
    if best_state is not None:
        ...
        best_model_path = os.path.join(BASE_DIR, "best_model_4layer.pt")
        torch.save(best_state, best_model_path)
        print(f"Saved best model state_dict to {best_model_path}")
    ...
    print(f"\n[TEST - 4-layer] Loss: {test_loss:.4f} | "
    ...
    ax.set(
        ...
        ylabel="True label",
        title=f"Confusion Matrix - 4-layer ({num_classes} Devices)"
    )
    plt.tight_layout()
    cm_path = os.path.join(BASE_DIR, "confusion_matrix_4layer.png")
    ...
    axes[0].set_title("Train / Val Loss - 4-layer")
    axes[0].legend()
    ...
    axes[1].set_title("Train / Val Macro-F1 - 4-layer")
    axes[1].legend()

    plt.tight_layout()
    curves_path = os.path.join(BASE_DIR, "train_curves_4layer.png")
